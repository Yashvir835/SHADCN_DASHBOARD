    ...
    if (selectedDocument && selectedAvatarName && selectedImage && selectedVoice) {
      ...
      try {
        const response = await fetch(
          `/api/experience-avatar?userId=${user?.id}&business=${encodeURIComponent(contextBusiness || "")}&avatarName=${encodeURIComponent(selectedAvatarName)}&image=${encodeURIComponent(selectedImage)}&voice=${encodeURIComponent(selectedVoice)}&userEmail=${encodeURIComponent(user?.emailAddresses[0]?.emailAddress || "")}`,
        // 'http://127.0.0.1:5000/experience-avatar'
        )
